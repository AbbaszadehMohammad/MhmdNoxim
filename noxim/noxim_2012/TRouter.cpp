  ...
  if(reset.read())
  ...
  else
    {
      ...
      // and wormhole related issues are addressed in the txProcess()
		...
		double  atimestamp;    // SC timestamp at packet generation
		atimestamp = sc_time_stamp().to_double()/1000;
		atimestamp = atimestamp - TGlobalParams::stats_warm_up_time;
	  ...
	  if ( !buffer[i].IsEmpty() )
	      ...
	      if (o != NOT_RESERVED)
		{
		  if ( current_level_tx[o] == ack_tx[o].read() )
			...
			{
				//hamid cout << "flit received = " << sc_time_stamp().to_double()/1000 << " " << stats.outBusy[o] << endl;
