...
//&&&&&
  unsigned int grp, grf;
  double gad, gat, gt, gmd, gp;
  grp = getReceivedPackets();
  grf = getReceivedFlits();
  gad = getAverageDelay();
  gat = getAverageThroughput();
  gt = getThroughput();
  gmd = getMaxDelay();
  gp = getPower();
bool golden = true;
  op.WriteOutputsToFile(golden,grp, grf,gad, gat, gt, gmd, gp );
  //&&&&&
  if (detailed)
    {
