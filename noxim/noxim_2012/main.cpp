...
double TGlobalParams::Locality						  = DEFAULT_LOCALITY;
...
void showHelp(char selfname[])
{
  ...
  cout << "\t\tlocal\t\tLocalized traffic distribution" << endl;
...
{cout <<"hi "<<TGlobalParams::packet_injection_rate <<endl;  
    // TEMP
    drained_volume = 0;
  ...
  else
  {
        ...
        if(new_pir>0)
	{
	  ...
	  else if(!strcmp(arg_vet[i+1],"local")) TGlobalParams::traffic_distribution = TRAFFIC_LOCALIZED;
	  else if(!strcmp(arg_vet[i+1],"table"))
	  {
      ...
      else if(!strcmp(arg_vet[i],"-locality"))
      {
        double lp = atof(arg_vet[i+1]);
        if(lp>=0)
	    {
          TGlobalParams::Locality = lp;
          i+=2;
        }
        else badArgument(arg_vet[i+1], arg_vet[i]);
	  }
...
cout << "locality " << TGlobalParams::Locality << endl;
