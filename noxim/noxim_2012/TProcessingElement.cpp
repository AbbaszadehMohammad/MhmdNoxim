...
void TProcessingElement::rxProcess()
{
  if(reset.read())
  {
	  //cout << sc_time_stamp().to_double()/1000 << " herep " << local_id<<endl;
    ack_rx.write(0);
    current_level_rx = 0;
  }
  else
...
void TProcessingElement::txProcess()
{
  ...
  else
  {
    ...
    if (canShot(packet))     //if false no packet send else canShot(packet)
    {
  ...
  if (TGlobalParams::traffic_distribution != TRAFFIC_TABLE_BASED)
	...
	{  
	  switch(TGlobalParams::traffic_distribution)
	    ...
	    case TRAFFIC_LOCALIZED:
	      packet = trafficLocalized();
	      break; 
...
int TProcessingElement::getPartion(int id)
{
	int row_part = ((id/(TGlobalParams::mesh_dim_x))/2) * (TGlobalParams::mesh_dim_x / 2);
	int column_part = (id % TGlobalParams::mesh_dim_y )/2;
	return row_part + column_part + 1;
}
//---------------------------------------------------------------------------

TPacket TProcessingElement::trafficLocalized()
{
  TPacket p;
  p.src_id = local_id;
  double rnd = rand()/(double)RAND_MAX;
  double localization = rand()/(double)RAND_MAX;
  double range_start = 0.0;
  int max_id = (TGlobalParams::mesh_dim_x * TGlobalParams::mesh_dim_y)-1;
  int arr [3];
  //cout << "\n " << sc_time_stamp().to_double()/1000 << " PE " << local_id << " rnd = " << rnd << endl;
if (localization <= TGlobalParams::Locality)
{
	for(int ii=0; ii<3; ii++)
		arr[ii] = 0;
	int index = 0;
	int part = getPartion(local_id);
	if ((local_id + 1 <= max_id) && (part == getPartion(local_id + 1)))
	{
		arr [index] = local_id + 1;
		index++;
	}
	else 
		if ((local_id - 1 >= 0) && (part == getPartion(local_id - 1)))
	   {
		   arr [index] = local_id - 1;
		   index++;
	   }
	//
	if ((local_id + TGlobalParams::mesh_dim_x <= max_id) && (part == getPartion(local_id + TGlobalParams::mesh_dim_x)))
	{
		arr [index] = local_id + TGlobalParams::mesh_dim_x;
		index++;
	}
	else 
		if ((local_id - TGlobalParams::mesh_dim_x >= 0) && (part == getPartion(local_id - TGlobalParams::mesh_dim_x)))
	   {
		   arr [index] = local_id - TGlobalParams::mesh_dim_x;
		   index++;
	    }
   	//
	if ((local_id + TGlobalParams::mesh_dim_x  -1 <= max_id) && (part == getPartion(local_id + TGlobalParams::mesh_dim_x - 1)))
	{
		arr [index] = local_id + TGlobalParams::mesh_dim_x - 1;
		index++;
	}
	
	else 
		if ((local_id - TGlobalParams::mesh_dim_x + 1 >= 0) && (part == getPartion(local_id - TGlobalParams::mesh_dim_x + 1)))
	    {
		   arr [index] = local_id - TGlobalParams::mesh_dim_x + 1;
		   index++;
	    }   
	...
	index = randInt(0 , 2);
	p.dst_id = arr[index];
	//cout <<" dst " << arr[index] << " src " << local_id << " part " << part << endl;
}
else
{
