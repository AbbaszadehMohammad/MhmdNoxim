			  ...
			  const TFlit& flit)
{
  ...
  //hamid 
  //cout << arrival_time << " " << warm_up_time << endl;
}
...
double TStats::getAverageThroughput()
{
  double sum = 0.0;
  double sum_flit = 0;
  double max_received_flit_time = 0.0;
  ...
  for (unsigned int k=0; k<chist.size(); k++)
    {
	  ...
	  //sum_flit = sum_flit + chist[k].total_received_flits;
	  //max_received_flit_time = (max_received_flit_time < chist[k].last_received_flit_time ? chist[k].last_received_flit_time : max_received_flit_time);
    }

  return sum;
  //return (max_received_flit_time); //>0 ? sum_flit / max_received_flit_time: -1);
}
		       ...
		       bool header)
{
  ...
  */
	TCoord t = 	id2Coord(curr_node);
  out << "node (" << t.x << "," << t.y << ")" << endl;
  out << "input totaltime = " << inTotaltime << "  output totaltime = " << outTotaltime << endl;
  for (unsigned int i=0; i<DIRECTIONS+1; i++)
    {
	  //out << "    DIRECTION " << i << ":" << setw(12) << inTotaltime << setw(12) << outTotaltime << endl;
	  out << "    DIRECTION " << i << ":" << setw(12) << getInBusy(i) << setw(12) << getOutBusy(i) << endl;
  }
  ...
  out << "% All received flits (flits): " << getReceivedFlits() << endl;
}

double TStats::getInBusy(unsigned int i)
{
	return (inTotaltime!=0 ? inBusy[i]/inTotaltime : -1);
}
double TStats::getOutBusy(unsigned int i)
{
	return (outTotaltime!=0 ? outBusy[i]/outTotaltime : -1);
}
//---------------------------------------------------------------------------
